...
def smooth_data_2d(data, window_length=5, polyorder=3, delay=1):
    ...
    return smoothed_data[-(delay + 1), :]


buffer_size = 50
data_buffer = np.zeros((buffer_size, 14))
...
def main(args):
    ...
    # reset_joints_left = np.deg2rad([-90, -5, -65, 5, 90, 90, 57])
    # reset_joints_right = np.deg2rad([90, 5, 65, -5, -90, -90, 57])
    #
    reset_joints = np.concatenate([reset_joints_left, reset_joints_right])
    curr_joints = env.get_obs()["joint_positions"]
    max_delta = (np.abs(curr_joints - reset_joints)).max()
    ...
    # model = Imitate_Model(ckpt_dir='./ckpt/0923diff_noaug_few', ckpt_name=model_name)  # camera pakage demo
    model = Imitate_Model(ckpt_dir='./ckpt', ckpt_name=model_name)  # camera pakage demo

    # model = Imitate_Model(ckpt_dir='./ckpt/demo_0812_mix', ckpt_name=model_name)  # camera pakage demo
    model.loadModel()
    ...
    with open(save_path, 'w', newline="") as file:
        ...
        writer.writerow(['episode'] + [f"action {i + 1}" for i in range(14)])

    # Initialize the parameters
    episode_len = 30000  # The total number of steps to complete the task. Note that it must be less than or equal to parameter 'episode_len' of the corresponding task in file 'ModelTrain.constants'
    t = 0
    last_time = 0
    ...
    model.qpos_history_raw[t] = observation['qpos']
    model.image_history.append(observation['images'])
    ...
    while t < episode_len:
        ...
        if args.show_img:
            ...
            imgs = np.hstack((observation['images']['top'], observation['images']['left_wrist'],
        ...
        if use_smooth:
            ...
            # time.sleep(0.05)
            if(flag):
                data_buffer[:-1, :] = data_buffer[1:, :]  # 将旧数据向后移动一行
                data_buffer[-1, :] = action0  # 添加新数据
            else:
                for i in range(data_buffer.shape[0]):
                    data_buffer[i,:]=action0
                    flag=1
            # delay is the level of smooth, higher level of delay will make action smoother, but less accuracy
            action = smooth_data_2d(data_buffer,window_length=5,polyorder=3,delay=2)
            action[6]=action0[6]
            action[13]=action0[13]
        # print('action0:',action0)
        print('action:',action)
        print("real action: ", action[0:7], action[8:14])
        # time.sleep(0.1)
        with open(save_path, 'a', newline="") as file:
            writer = csv.writer(file)
            writer.writerow([t] + [action])
