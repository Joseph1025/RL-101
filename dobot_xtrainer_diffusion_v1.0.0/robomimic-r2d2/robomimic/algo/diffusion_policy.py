...
class ConditionalUnet1D(nn.Module):
    ...
    def forward(self, 
        ...
        for idx, (resnet, resnet2, downsample) in enumerate(self.down_modules):
            ...
            h.append(x)
            x = downsample(x)
            # print(f"shape of downspamle x is {x.shape}")
        ...
        for idx, (resnet, resnet2, upsample) in enumerate(self.up_modules):
            ...
            # print(f"shape of h.pop() is {h[-1].shape}")
            # if h[-1].shape[-1] < x.shape[-1]:
            #     # Apply padding to the last dimension of h[-1]
            #     padding_size = x.shape[-1] - h[-1].shape[-1]
            #     h[-1] = F.pad(h[-1], (0, padding_size), "constant", 0)#
            # print(f"before cat, x: {x.shape}, h[-1]: {h[-1].shape}")
            # if x.shape[2] > h[-1].shape[2]:
            #     x = x[:, :, :h[-1].shape[2]]
            x = torch.cat((x, h.pop()), dim=1)
            # print(f"after cat, x: {x.shape}")
            x = resnet(x, global_feature)
            x = resnet2(x, global_feature)
            x = upsample(x)
            
            # print(f"shape of upsample x is {x.shape}")
